...
abstract class ReplaySpec2 extends WordSpec with MustMatchers {
  type FixtureParam = Fixture

  val Increment = EventClass("increment")
  ...
  "A journal" when {
    "used for snapshotted replay" must {
      "replay to current state from latest snapshot" in { fixture =>
        ...
        target.receivedOffer.get.snapshot must be(Snapshot(1, 4, ss12.timestamp, SnapshotClass(4)))
      ...
      "replay to current state from selected snapshot (sequence number criteria don't include latest snapshot)" in { fixture =>
        ...
        target.awaitReplayProcessed
        target.receivedOffer.get.snapshot must be(Snapshot(1, 2, ss11.timestamp, SnapshotClass(2)))
        target.receivedMessages.map(_.withTimestamp(0L)) must be(List(
...
abstract class PersistentReplaySpec2 extends ReplaySpec2 {
  "recognize stored snapshots when started" in { fixture =>
    ...
    anotherTarget1.receivedOffer.get.snapshot must be(Snapshot(1, 4, ss12.timestamp, SnapshotClass(4)))
    anotherTarget1.receivedMessages.map(_.withTimestamp(0L)) must be(List(
      Message(Increment, 5, 0L, 1)))

    anotherTarget2.awaitReplayProcessed
    anotherTarget2.receivedOffer.get.snapshot must be(Snapshot(1, 2, ss11.timestamp, SnapshotClass(2)))
