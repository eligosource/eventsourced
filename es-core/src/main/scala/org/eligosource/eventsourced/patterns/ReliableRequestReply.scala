...
/**
 * Reliable request-reply channel support.
 */
trait ReliableRequestReply {
   ...
   *    request sender.
   *  - sends a special [[org.eligosource.eventsourced.patterns.DestinationNotResponding]]
   *    reply to the request sender if the destination doesn't reply within  `replyTimeout`.
   *  - sends a special [[org.eligosource.eventsourced.patterns.DestinationFailure]] reply
